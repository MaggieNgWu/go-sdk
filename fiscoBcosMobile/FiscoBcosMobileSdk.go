...
func DeployContract(abiContract string, binContract string, param string) *DeployContractResult {
...
func toReceipt(_r *types.Receipt) (*TxReceipt, error) {
	...
	for _, one := range _r.Logs {
		...
		if err != nil {
			...
			log = EventLog{one.Removed, one.LogIndex, one.TransactionIndex, one.TransactionHash, one.BlockHash, one.BlockNumber, one.Address, one.Data, one.Type, topics}
		}
...
// string to interface
func StringToInterface(paramType string, value string) (interface{}, error) {
...
func stringToInterfaceBasic(paramType string, value string) (interface{}, error) {
	switch paramType {
	case "string":
		return value, nil
	case "int":
	...
	case "uint64":
		return strconv.ParseUint(value, 10, 64)
	case "uint256":
		in, err := strconv.ParseUint(value, 10, 64)
		if err != nil {
			return nil, err
		}
	...
	case "[]byte", "bytes":
		byteValue := common.FromHex(value)
